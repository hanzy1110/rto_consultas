...
def plot_p(summs, key, ax, label):
    ...
    ax.plot(years, means+sds, ls="--", color="red")
    ax.plot(years, means-sds, ls="--", color="blue")
    ax.fill_between(years, means-sds, means+sds, alpha = 0.4, label=r"$1\sigma$")
...
def plot_densities(sample_dict, ax, label, set_legend=False):
    ...
    #     sns.kdeplot(data=samples, ax=ax, label=label_parsed, fill=True, color=color, legend=False)
    ax = sns.kdeplot(data=pd.DataFrame.from_dict(sample_dict),
                ax=ax,
                fill=True,
                palette="crest",
                legend=set_legend,
    ...
    ax.set_title(label)
    if set_legend:
        sns.move_legend(ax, "upper left", bbox_to_anchor=(0.0,-0.1), ncols=7)
...
if __name__ == "__main__":
    ...
    fig2.set_size_inches(6.3,3.5)
    fig.subplots_adjust(wspace=0., hspace=1.0)

    plot_densities(p_ap_dict, axs2[0], label = r"$P_{Ap}$")
    plot_densities(p_rele_dict,axs2[1], label = r"$P_{AC}$")
    plot_densities(p_regr_dict,axs2[2], label = r"$P_{Re}$", set_legend=True)
    # plt.legend()
