...
def handle_nrocertificados(
    ...
    if model == CccfCertificados and not check_for_empty_query(nrocertificado_init):
        ...
        queryset = model.objects.filter(nrocertificado=int(nrocertificado_init[0]))
        return queryset
...
def handle_initial_hab_form(dominio):
    ...
    try:
        ...
        cccf = cccf.order_by("-idcertificado").values_list("nrocertificado", flat=True)[
            0
        ]
...
def get_resumen_data_mensual(cleaned_data, tipo_uso=None):
    ...
    rev_intersection = verificaciones_a_cobrar.intersection(v_reverificado_a_cobrar)
    len_v_a_cobrar = len(verificaciones_a_cobrar)
    ...
    logger.info(f"VERIFICACIONES AGREGADAS len => {len_v_a_cobrar - len(v_reverificado_a_cobrar)}")
